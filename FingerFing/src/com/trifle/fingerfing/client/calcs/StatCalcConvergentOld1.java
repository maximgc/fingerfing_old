...
public class StatCalcConvergentOld1 implements StatCalc {
	...
	private void calcLastMeanInTempo(double lastMeanSpeed) {
		// на базе средне квадратичного отклонения, нормализованного по текущему
		// среднему, и увеличена чувствительность как 1/x^5
		sumQuadDeviation = (sumQuadDeviation + Math.pow(2.0 * stepSpeed
				/ (stepSpeed + lastMeanSpeed) - 1, 2.0)) / 2;
	...
	private void calcLastMeanSpeed() {
		...
		if (lastMeanSpeed > lastMaxMeanSpeed)
			lastMaxMeanSpeed = lastMeanSpeed;

		// вариант попытки стабилизировать от разовых скачков,
		// плохо переносит разовые падения скорости, например можно набирать по
		// 3 быстрых нажатия с пред подготовкой но длинными перерывами - средняя
		// будет высокой, это не справедливо
		// double cof = 1.0/Math.pow(Math.abs(stepSpeed -
		// lastMeanSpeed)/lastMeanSpeed+1, 2.0);
		// lastMeanSpeed =
		// (lastMeanSpeed==0)?stepSpeed:(1-cof)*lastMeanSpeed+cof*stepSpeed;
	}

	private double round001(double d) {
		//TODO  может есть более красивое решение?
		return Math.round(d * 100.0) / 100.0; 
	}

	@Override
	public void addStep(long stepTime, int eval) {
	...
	public String toDebugString() {
		return "count=" + fullCount + "countSuccess=" + fullCountSuccess
				+ " sumTime=" + fullTime + " lastStepTime=" + stepTime;
	}
