		...
		final List<Set<NativeKey>> exisizeSets = new ArrayList<Set<NativeKey>>() {
			{
				...
				add(EnumSet.of(NativeKey.KEY_R, NativeKey.KEY_T,
						NativeKey.KEY_Y, NativeKey.KEY_U));
				add(EnumSet.of(NativeKey.KEY_V, NativeKey.KEY_B,
						NativeKey.KEY_N, NativeKey.KEY_M));
			}
		};
		...
		fp.addKeyDownHandler(new KeyDownHandler() {
			...
			public void onKeyDown(KeyDownEvent event) {
				...
				keyWidget.setEffect(
						...
						effectPress);
				statCalc.addRecord(System.currentTimeMillis(), ev.check(curKey,
						NativeKey.getByNativeCode(event.getNativeKeyCode())));

				sd.setData(statCalc.calcAllTime(),
						statCalc.calcCurrentMeanSpeed(),
						statCalc.calcDensityError(),
						statCalc.calcFullMeanSpeed(), statCalc.calcCount(),
						statCalc.calcMeanDeviation(),
